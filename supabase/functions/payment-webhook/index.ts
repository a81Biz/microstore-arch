...
class PaymentWebhookController extends BaseController {
  ...
  private async handlePayPalWebhook(body: string, headers: Headers) {
    ...
    if (!webhookId || !clientId || !clientSecret) {
      throw new BusinessError('CONFIG_ERROR', 'Credenciales PayPal no configuradas', 500);
    }

    // Verificar vía PayPal Webhooks Verification API
    ...
    if (event.event_type === 'CHECKOUT.ORDER.APPROVED') {
      ...
      const amountCents = Math.round(parseFloat(unit.amount.value) * 100);
      return this.handlePaymentSuccess(
        unit.reference_id,
        event.resource.id,
        'paypal',
